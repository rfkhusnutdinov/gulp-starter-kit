...
- По возможности минимально использовать `@media (width < ...) || @media (width >= ...)` и для адаптивности использовать функции `fluid` и `fluid-reverse` для адаптивных значений через `clamp`
